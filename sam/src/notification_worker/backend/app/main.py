...
async def lifespan(app: FastAPI):
    ...
    # OpenSearch Index Verification (Phase 8.6, non-blocking on startup)
    try:
        from app.services.search.opensearch_service import opensearch_service
        opensearch_service.ensure_index()
    except Exception as exc:
        logger.warning("Startup OpenSearch index verification notice (non-blocking): %s", exc)
...
def get_root():
    ...
    return {
        ...
        "version": settings.APP_VERSION,
        "phase": "Phase 8.6 - OpenSearch Authorized Civic Docket Search",
        "status": "operational",

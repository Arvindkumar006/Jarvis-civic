...
class CedarService:
    ...
    def _build_entities(self, auth_req: AuthorizationRequest) -> List[Dict[str, Any]]:
        ...
        # 1. Principal Entity
        principal_type = f"JarvisCivic::{principal.cedar_entity_type}"
        principal_attrs: Dict[str, Any] = {}
        if principal.role in (ApplicationRole.AUTHORITY_OFFICER, ApplicationRole.MUNICIPAL_SUPERVISOR):
            principal_attrs["department"] = principal.department or ""

        entities.append({
            "uid": {"type": principal_type, "id": principal.principal_id},
        ...
        if auth_req.resource_type == "CivicCase":
            ...
            case_attrs: Dict[str, Any] = {
                ...
                "department": auth_req.resource_department or "",
                "status": auth_req.resource_status or "DRAFT",
                "is_public": bool(auth_req.is_public),
            }

            entities.append({
                "uid": {"type": "JarvisCivic::CivicCase", "id": auth_req.resource_id},
                "attrs": case_attrs,
                "parents": [],
            })
            ...
            entities.append({
                "uid": {"type": "JarvisCivic::AuditRecord", "id": auth_req.resource_id},
                "attrs": {
                    "department": auth_req.resource_department or "",
                },
                "parents": [],
